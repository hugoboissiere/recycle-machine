...
recycleMachine = RegisterMod("recycleMachine", 1)
recycleMachineId = Isaac.GetEntityVariantByName("Recycle machine")
game = Game()
player = Game():GetPlayer(0)
nbOfItems = Isaac.GetItemConfig():GetCollectibles().Size
function postUpdate(self)
    local room = game:GetRoom()
    local entities = room:GetEntities()
    local recycleMachineInstance = nil
    do
        local index = 0
        while index < entities.Size do
            local element = entities:Get(index)
            if element and ((element.Type ~= EntityType.ENTITY_PLAYER) and (element.Type ~= EntityType.ENTITY_EFFECT)) then
                if (element.Type == EntityType.ENTITY_SLOT) and (element.Variant == 294) then
                    recycleMachineInstance = element
                end
            end
            if (recycleMachineInstance ~= nil) and (player ~= nil) then
                local c = recycleMachineInstance.Position
                c.X = recycleMachineInstance.Position.X - player.Position.X
                c.Y = recycleMachineInstance.Position.Y - player.Position.Y
                if c:Length() < (recycleMachineInstance.Size + player.Size) then
                    local ownedItems = {}
                    do
                        local index = 0
                        while index < nbOfItems do
                            if player:HasCollectible(index, true) then
                                __TS__ArrayPush(ownedItems, index)
                            end
                            if (ownedItems ~= nil) and (#ownedItems > 0) then
                                local itemToRemove = math.floor(
                                    math.random() * (((#ownedItems - 1) - 0) + 1)
                                ) + 0
                                Isaac.DebugString(
                                    "item to remove" .. tostring(itemToRemove)
                                )
                                player:RemoveCollectible(ownedItems[itemToRemove + 1])
